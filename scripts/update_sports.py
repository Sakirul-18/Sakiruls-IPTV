...
SOURCE_PRIORITIES = {
    ...
    "fancode": {"lock": [FANCODE_EXCLUSIVE_SOURCE]},
...
def parse_m3u(content: str, source_url: str = "") -> List[ChannelData]:
    ...
    for line in content.splitlines():
        ...
        if line.startswith("#EXTINF"):
            ...
            # --- FANCODE DYNAMIC EVENT FIX ---
            # If group-title indicates FanCode (e.g. "Fancode-Cricket"), map it to the static master name
            if "fancode" in current_group.lower():
                match_name = current_group.replace("-", " ") 
            else:
...
def pick_best_stream(candidates: List[ChannelData], preferred_sources: List[str], session: requests.Session) -> Optional[ChannelData]:
    ...
    for cand in candidates:
        ...
        # Prevents assigning the same live event URL to duplicate master placeholders
    ...
    # Lock this URL globally so it isn't given to the next duplicate placeholder
...
def main():
    ...
    for ch_name in CHANNELS:
        ...
        if best_stream:
            ...
            # Inject actual event name for FanCode streams, keep placeholder names for normal channels
            display_name = best_stream.name if "fancode" in best_stream.group.lower() else ch_name.strip()
